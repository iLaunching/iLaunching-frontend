/**
 * Smart Router Node Renderer - Circular Design with Retina-grade rendering
 * Renders circular node with nested layers and routing indicator
...
export class SmartRouterNodeRenderer {
  ...
  public render(
    ...
    try {
      ...
      // Router Indicator (center diamond with routing symbol)
      this.renderRouterIndicator(ctx, centerX, centerY, zoom);
  ...
  /**
   * Render Router Indicator - static white diamond with routing symbol
   */
  private renderRouterIndicator(
    ctx: CanvasRenderingContext2D,
    centerX: number,
    centerY: number,
    zoom: number
    ...
    // Draw white routing symbol (⇄)
    ctx.save();
    ctx.fillStyle = '#ffffff';
    ctx.font = `bold ${50 * zoom}px "Arial", sans-serif`;
